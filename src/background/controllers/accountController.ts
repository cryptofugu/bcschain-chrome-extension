...
import { Wallet as BCSWallet } from 'bcsjs-wallet';
import assert from 'assert';

import BCSChromeController from '.';
import IController from './iController';
import { MESSAGE_TYPE, STORAGE, NETWORK_NAMES, BCSCHROME_ACCOUNT_CHANGE } from '../../constants';
...
export default class AccountController extends IController {
  ...
  */
  public addAccountAndLogin = async (accountName: string, privateKeyHash: string, wallet: BCSWallet) => {
  ...
  */
  public saveToFile = (accountName: string, mnemonic: string) => {
    ...
    element.download = `bcschrome_${accountName}_${timestamp}.bak`;
    element.click();
  ...
  */
  public onAccountLoggedIn = async (isSessionRestore = false) => {
     ...
     * reopening the popup, we don't need to send the SEND_INPAGE_BCSCHROME_ACCOUNT_VALUES event to
     * the inpage because window.bcschrome.account has not changed.
     */
    if (!isSessionRestore) {
      this.main.inpageAccount.sendInpageAccountAllPorts(BCSCHROME_ACCOUNT_CHANGE.LOGIN);
    }
  ...
  */
  private recoverFromPrivateKeyHash(privateKeyHash: string): BCSWallet {
  ...
  private getPrivateKeyHash(wallet: BCSWallet) {
  ...
  */
  private getWalletInfo = async (sendInpageUpdate = true) => {
    ...
    if (infoDidUpdate) {
      ...
      if (sendInpageUpdate) {
        this.main.inpageAccount.sendInpageAccountAllPorts(BCSCHROME_ACCOUNT_CHANGE.BALANCE_CHANGE);
      }

      this.updateAndSendMaxBCSAmountToPopup();
    }
  }
  ...
  * @param receiverAddress The address to send BCS to.
  ...
  */
  private sendTokens = async (receiverAddress: string, amount: number, transactionSpeed: TRANSACTION_SPEED) => {
    ...
    */
    try {
      ...
      const feeRate = rates[transactionSpeed]; // satoshi/byte; 500 satoshi/byte == .005 BCS/KB
      if (!feeRate) {
  ...
  /**
   * We update the maxBCS amount under 2 scnearios
   * 1 - When wallet.info has been updated because a new balance has a new maxBCSSend
   * 2 - Whenever the maxBCSSend is requested, because even if the balance does not
   * change, the available UTXOs can change(which causes a change in maxBCSSend).
   * For instance when a user sends BCS, but that transaction has not confirmed yet,
   * the balance will not change, but calcMaxBCSSend is able to account for those
   * unconfirmed UTXOs and update maxBCSSend accordingly.
   */
  private updateAndSendMaxBCSAmountToPopup = async () => {
    if (!this.loggedInAccount || !this.loggedInAccount.wallet || !this.loggedInAccount.wallet.qjsWallet) {
      throw Error('Cannot calculate max balance with no wallet instance.');
    }

    const calcMQSPr = this.loggedInAccount.wallet.calcMaxBCSSend(this.main.network.networkName);
    calcMQSPr.then(() => {
      chrome.runtime.sendMessage({ type: MESSAGE_TYPE.GET_MAX_BCS_SEND_RETURN,
        maxBCSAmount: this.loggedInAccount!.wallet!.maxBCSSend });
    });
  }
    ...
    try {
      switch (request.type) {
        ...
        case MESSAGE_TYPE.GET_WALLET_INFO:
          ...
          break;
        case MESSAGE_TYPE.GET_BCS_USD:
          sendResponse(this.loggedInAccount && this.loggedInAccount.wallet
            ? this.loggedInAccount.wallet.bcsUSD : undefined);
          break;
        case MESSAGE_TYPE.VALIDATE_WALLET_NAME:
          sendResponse(this.isWalletNameTaken(request.name));
          break;
        case MESSAGE_TYPE.GET_MAX_BCS_SEND:
          this.updateAndSendMaxBCSAmountToPopup();
          break;
        default:
          break;

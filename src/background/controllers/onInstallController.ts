...
export default class OnInstallController extends IController {
  constructor(main: BCSChromeController) {
    ...
    * dapp apps using BCSChrome. We want to refresh these tabs so that they can
    ...
    * injected scripts from prior versions of BCSChrome.
    * To get refreshed, dapp tabs must implement the
    * handleBCSChromeInstallOrUpdate event listener described in the Readme.
    */
  ...
  private refreshTab(tab: chrome.tabs.Tab) {
     // Tells the content script to post a msg to the inpage window letting it know that BCSChrome was installed or updated.
     chrome.tabs.executeScript(tab.id!, {code:
      `window.postMessage(
        {
          message: { type: 'BCSCHROME_INSTALLED_OR_UPDATED' }
        },
        '*'
      )`,
    });
